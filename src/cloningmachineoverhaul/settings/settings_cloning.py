...
class CloningSettings:
    def __init__(self,
                 age: int = 0,
                 gender: int = 0,
                 relationship_bit: int = RelationshipBit.SIBLING,
                 sex: int = Sex.AUTOMATIC,
                 genitalia: int = Genitalia.AUTOMATIC,
                 fertility: int = Fertility.AUTOMATIC,
                 sexual_preference: int = SexualPreference.AUTOMATIC,
                 voice_actor: int = VoiceActor,
                 voice_pitch: int = 0,
                 voice_effect: int = 0):
        ...
        self._log = M22LogFileManager(LOG_FILENAME_PREFIX)
        ...
        self.tunable_clone_as_offspring = get_tuning(TuningId.CMO_CLONE_SIM_PICKED_CONTINUATION).outcome._success_actions.basic_extras[1]._tuned_values.set_genealogy
    ...
    def _change_relbits(self):
        ...
        if tuning is not None:
            if self.relationship_bit == RelationshipBit.OFFSPRING:
                ...
                inject_to_continuation([family_offspring_rel_bit, clone_rel_bit], genealogy=self.tunable_clone_as_offspring)
                ...
                inject_to_continuation([family_sibling_rel_bit, clone_rel_bit])
                ...
                inject_to_continuation([clone_rel_bit])
